...
const Home = () => {
  ...
  // Sửa hàm xác định dashboard link theo roles (mảng)
  const getDashboardLink = () => {
    if (!currentUser) return '/dashboard';
    const roles = (currentUser.roles || []).map(r => r.toLowerCase());
    if (roles.some(r => r.includes('admin'))) return '/admin/dashboard';
    if (roles.some(r => r.includes('manager'))) return '/manager/dashboard';
    if (roles.some(r => r.includes('medical'))) return '/medical/health-checkups';
    if (roles.some(r => r.includes('parent'))) return '/parent/dashboard';
    if (roles.some(r => r.includes('student'))) return '/dashboard'; // hoặc '/student/profile'
    return '/dashboard';
  };
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: theme.palette.background.default, color: theme.palette.text.primary }}>
        ...
        <Container maxWidth="lg">
          <Grid container spacing={4} alignItems="center">
            <Grid item xs={12} md={8}>
              ...
              <Typography variant="h5" sx={{ mb: 3, opacity: 0.9, color: theme.palette.text.secondary }}>
                Comprehensive health tracking and management for {schoolInfo.name}
              </Typography>
              <Typography variant="body1" sx={{ mb: 4, opacity: 0.8, color: theme.palette.text.secondary }}>
            ...
            <Grid item xs={12} md={4}>
              <Box sx={{ textAlign: 'center' }}>
                <SchoolIcon sx={{ fontSize: 200, opacity: 0.2, color: theme.palette.text.primary }} />
              </Box>
            </Grid>
          </Grid>
        </Container>
      ...
      <Container maxWidth="lg" sx={{ py: 6 }}>
            ...
            <Grid item xs={12} md={4}>
              <Box sx={{ textAlign: 'center' }}>
                ...
                <Button
                  ...
                  color="primary"
                  fullWidth
                  sx={{ borderColor: theme.palette.primary.main, color: theme.palette.primary.main, bgcolor: 'transparent', '&:hover': { bgcolor: theme.palette.action.hover } }}
                >
            ...
            <Grid container spacing={3}>
              {quickActions.map((action, index) => (
                <Grid item xs={12} sm={6} md={3} key={index}>
                  <Card sx={{ height: '100%', bgcolor: theme.palette.card.main, color: theme.palette.text.primary, '&:hover': { boxShadow: 6 }, transition: 'box-shadow 0.3s', border: `1px solid ${theme.palette.divider}` }}>
        ...
        {/* Features Section */}
        <Box sx={{ mb: 6 }}>
          <Typography variant="h4" sx={{ fontWeight: 600, mb: 3, textAlign: 'center', color: theme.palette.text.primary }}>
            Platform Features
          </Typography>
          <Grid container spacing={4}>
            {features.map((feature, index) => (
              <Grid item xs={12} md={6} lg={4} key={index}>
                <Card sx={{ height: '100%', bgcolor: theme.palette.card.main, color: theme.palette.text.primary, '&:hover': { boxShadow: 6 }, transition: 'box-shadow 0.3s', border: `1px solid ${theme.palette.divider}` }}>
                  <CardContent sx={{ textAlign: 'center' }}>
                    <Box sx={{ mb: 2 }}>
                      {React.cloneElement(feature.icon, { color: 'primary', sx: { fontSize: 40 } })}
                    </Box>
        ...
        >
          <Typography variant="h5" sx={{ fontWeight: 600, mb: 3, color: theme.palette.success.dark }}>
            Daily Health Tips
          </Typography>
          <Grid container spacing={3}>
            <Grid item xs={12} md={4}>
              <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                <CheckIcon sx={{ color: theme.palette.success.main, mt: 0.5 }} />
                <Box>
