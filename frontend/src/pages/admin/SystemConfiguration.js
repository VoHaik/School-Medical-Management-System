...
const SystemConfiguration = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        ...
        setSnackbar({ open: true, message: 'Failed to fetch system configuration', severity: 'error' });
      }
    };
    fetchConfig();
  }, []);
  ...
  const handleSaveSettings = async (section, data) => {
    try {
      const newSettings = { ...systemSettings, [section]: { ...systemSettings[section], ...data } };
      await updateSystemConfig(newSettings);
      setSystemSettings(newSettings);
      setSnackbar({ open: true, message: 'Settings saved successfully', severity: 'success' });
      setConfigDialog({ open: false, section: '', title: '' });
    } catch (error) {
      setSnackbar({ open: true, message: 'Failed to update system configuration', severity: 'error' });
    }
  };
  ...
  if (!systemSettings) {
    return <Typography>Loading system configuration...</Typography>;
  }
