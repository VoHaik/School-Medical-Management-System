...
const AppMenu = ({ isOpen, closeMenu }) => {
  ...
  const getNavigationItems = () => {
    ...
    switch (primaryRole) {
      ...
      case 'ROLE_ADMIN':
        items = [
          ...
          { path: '/admin/analytics-reports', label: 'Analytics & Reports', icon: 'fas fa-chart-line' },
          ...
          { path: '/admin/medical-events', label: 'Medical Events', icon: 'fas fa-ambulance' },
        ];
        break;
      case 'ROLE_STUDENT':
        items = [
    ...
    // Add Profile link cho các user KHÔNG phải admin
    if (primaryRole !== 'ROLE_ADMIN' && !items.find(item => item.path === '/profile' || item.path === '/student/profile')) {
