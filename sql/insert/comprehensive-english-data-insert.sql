-- =============================================
-- Comprehensive English Data Insert Script
...
-- Check for table existence and use correct table name
...
END
ELSE IF OBJECT_ID('roles', 'U') IS NOT NULL
BEGIN
    -- Clear existing data if needed
    DELETE FROM roles WHERE role_name IN ('ROLE_ADMIN', 'ROLE_NURSE', 'ROLE_PARENT', 'ROLE_STUDENT', 'ROLE_MANAGER');
...
ELSE
BEGIN
    PRINT 'ERROR: Roles table not found in database!';
    PRINT 'Please ensure the database schema is properly created.';
    RETURN;
END
...
-- Get Role IDs (check both possible table names)
...
-- Check if ParentStudentRelationships table exists and clear existing relationships
IF OBJECT_ID('ParentStudentRelationships', 'U') IS NOT NULL
BEGIN
    DELETE FROM ParentStudentRelationships;
    
    -- Insert parent-student relationships using correct column names
    INSERT INTO ParentStudentRelationships (parent_code, student_code, relationship_type, created_at) VALUES
...
END
ELSE IF OBJECT_ID('parent_student_relationships', 'U') IS NOT NULL
BEGIN
    DELETE FROM parent_student_relationships;
    
    -- Try alternative table name with snake_case
...
ELSE
BEGIN
    PRINT 'Warning: Parent-Student relationship table not found. Skipping relationship insertion.';
    PRINT 'This may be normal if relationships are managed differently in your database schema.';
END
...
INSERT INTO BlogPosts (title, content, summary, category_id, user_id, created_at, updated_at) VALUES
...
(@ParentUser1, (SELECT student_code FROM Students WHERE student_code = 'ST2025001'), 'Reminder: Annual health screenings will begin on September 15th. Please ensure your child has completed all required forms and has eaten breakfast before their appointment.', 'HEALTH_REMINDER', 0, GETDATE(), '/health-events'),

(@ParentUser1, (SELECT student_code FROM Students WHERE student_code = 'ST2025001'), 'The annual influenza vaccination campaign will begin on November 1st in the school gymnasium. Consent forms must be returned by October 25th.', 'VACCINATION_NOTICE', 0, GETDATE(), '/vaccinations'),

(@ParentUser2, (SELECT student_code FROM Students WHERE student_code = 'ST2025002'), 'Your medication request for emergency albuterol inhaler has been approved. Please review the administration guidelines and contact the health office with any questions.', 'MEDICATION_UPDATE', 0, GETDATE(), '/medication-requests'),

(@ParentUser3, (SELECT student_code FROM Students WHERE student_code = 'ST2025011'), 'Your child''s diabetes management plan has been updated. Please review the new glucose monitoring schedule and contact the school nurse with any questions.', 'MEDICAL_UPDATE', 1, GETDATE(), '/health-records'),
