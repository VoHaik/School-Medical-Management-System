...
public interface StudentHealthCheckupRepository extends JpaRepository<StudentHealthCheckup, Integer> {
    ...
    List<StudentHealthCheckup> findByHealthEvent_EventId(Integer eventId);
    Optional<StudentHealthCheckup> findByHealthEvent_EventIdAndStudent_StudentCode(Integer eventId, String studentCode);
    ...
    @Query("SELECT shc FROM StudentHealthCheckup shc WHERE shc.healthEvent.eventId = :eventId AND shc.followUpNeeded = true")
